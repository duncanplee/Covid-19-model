...
#### Check on the rho arguments
  ...
  if(alpha<0 ) stop("rho.T is outside the range [0, 1].", call.=FALSE)  
  if(alpha>1 ) stop("rho.T is outside the range [0, 1].", call.=FALSE)  
...
summary.beta <- cbind(summary.beta, effectiveSize(samples.beta.orig), geweke.diag(samples.beta.orig)$z)
...
colnames(summary.beta) <- c("Median", "2.5%", "97.5%", "n.effective", "Geweke.diag")
  
summary.hyper <- array(NA, c((J+2) ,5))
...
summary.hyper[1:J, 4] <- diag(apply(samples.Sigma, c(2,3), effectiveSize))
    for(r in 1:J)
    {
    summary.hyper[r, 5] <- geweke.diag(samples.Sigma[ ,r,r])$z    
    }
  
    if(!fix.rho.S)
    {
    summary.hyper[(J+1), 1:3] <- quantile(samples.rho, c(0.5, 0.025, 0.975))
    summary.hyper[(J+1), 4:5] <- c(effectiveSize(samples.rho), geweke.diag(samples.rho)$z)
    }else
    {
    ...
    summary.hyper[(J+2), 4:5] <- c(effectiveSize(samples.alpha), geweke.diag(samples.alpha)$z)
    }else
    {
