...
summary.hyper <- array(NA, c((J+3) ,5))
...
summary.hyper[1:J, 4] <- diag(apply(samples.Sigma, c(2,3), effectiveSize))
  ...
  summary.hyper[(J+2), 4:5] <- c(effectiveSize(samples.alpha[ ,1]), geweke.diag(samples.alpha[ ,1])$z)
  summary.hyper[(J+3), 1:3] <- quantile(samples.alpha[ ,2], c(0.5, 0.025, 0.975))
  summary.hyper[(J+3), 4:5] <- c(effectiveSize(samples.alpha[ ,2]), geweke.diag(samples.alpha[ ,2])$z)
  }else
  {
